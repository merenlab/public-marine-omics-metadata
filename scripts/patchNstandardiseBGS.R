...
# Define the count_unique_entries function
count_unique_entries <- function(df, column_name) {
  return(length(unique(df[[column_name]])))
}
...
BGS_pub_merged_patch2 <- BGS_pub_merged_patch2 %>%
  mutate(
    # Parse the datetime string using lubridate's ymd_hms
    collection_datetime = ymd_hms(collection_date, tz = "UTC"),
    year = year(collection_datetime),
    month = month(collection_datetime),
    day = day(collection_datetime),
    time = format(collection_datetime, "%H:%M:%S"),
    collection_date = as.Date(collection_datetime)
  )
...
BGS_pub_merged_patch2$collection_date
...
BGS_pub_merged_patch2$env_feature <- NA
BGS_pub_merged_patch2$env_biome <- NA
BGS_pub_merged_patch2$env_material <- NA
